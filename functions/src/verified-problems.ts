...
export function parseVerifiedProblem(question: string, topic: string): VerifiedGivens {
...
export function solveVerifiedProblem(givens: VerifiedGivens): PrivateProblemReference {
  ...
  else if (operation === "mode") { const counts = new Map<number, number>(); values.forEach(v => counts.set(v,(counts.get(v)??0)+1)); const max = Math.max(...counts.values()); result = max === 1 ? [] : [...counts].filter(([,count])=>count===max).map(([v])=>v).sort((a,b)=>a-b); setup = "Count each value's frequency. If all appear once, the mode set is empty."; }
